        ...
        <>
            {
                isloading ? <Loader /> :
                    (<div className="my-6" id="food">
                        ...
                        <div className="mx-auto max-w-3xl lg:max-w-4xl xl:max-w-7xl py-8 sm:py-16 px-4 sm:px-8 lg:px-8" ref={foodSectionRef}>
                            ...
                            {filteredFoods.length === 0 ? (
                                ...
                                <>
                                    <div className="grid gap-x-4 gap-y-6 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
                                            ...
                                            <div key={food._id} className="group cursor-pointer" data-aos="fade-up" data-aos-duration={`${1500 + index * 100}`} data-aos-delay={`${index * 100}`} data-aos-easing="ease" data-aos-anchor-placement="top-bottom" >
                                                <Link to={`/food/${food._id}`} className="btn w-full">
                                                    <div className="relative">
                                                        ...
                                                        }} className={`absolute top-2 right-2 text-white z-10 cursor-pointer hover:scale-[1.1] transition-all`} >
                                                            <i className={`${(favourites || []).includes(food._id) ? 'fa-solid text-red-500 hover:text-red-600 fa-beat' : 'fa-regular'} fa-heart fa-lg`}></i>
                                                        </button>
                                                    </div>
                                                    <h3 className="mt-2 sm:mt-4">{food.name}</h3>
                                                    <div className="mt-2 flex gap-0.5 text-sm sm:text-lg font-medium flex-wrap">
                                                        <s className="text-red-500">₹{Math.ceil(food.price + (food.price * 0.10))}/-</s>
                                                        <p data-aos="fade-right">₹{food.price}/- </p>
                                                    </div>
                                                </Link>
                                            </div>
                                        ))}
