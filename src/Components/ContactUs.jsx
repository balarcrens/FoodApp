...
export default function ContactUs() {
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e) => {
            ...
            const rotateX = ((centerY - y) / centerY) * 40;
        ...
        <>
            ...
            <div className="my-15 flex flex-wrap justify-center">
                <div className='flex justify-center items-center' data-aos="fade-right" data-aos-duration="1000">
                    <img className='' ref={imgRef} src="/images/contactus.png" alt="" style={{ transform: "perspective(1000px) rotateY(0deg) rotateX(0deg)", transition: "transform 0.1s ease-out", transformStyle: "preserve-3d" }} />
                </div>
                <form method="POST" onSubmit={handlesubmit} className="my-5 px-5 max-w-xl" data-aos="fade-left" data-aos-duration="1000">
                    <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
                        ...
                        <div className="sm:col-span-2">
                            <label htmlFor="phone" className="block text-sm/6 font-semibold "> Phone number </label>
                            <div className="mt-2.5">
                                <div className="flex rounded-md outline-1 -outline-offset-1 outline-gray-300 has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-indigo-600">
                                    <div className="grid shrink-0 grid-cols-1 focus-within:relative">
                                        <select id="country" name="country" autoComplete="country" aria-label="Country" className="col-start-1 row-start-1 w-full appearance-none rounded-md py-2 pr-7 pl-3.5 text-base placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6" >
                                            <option>IND</option>
                                            <option>US</option>
                                            <option>EU</option>
                                        </select>
                        ...
                        <Field className="flex gap-x-4 sm:col-span-2">
                            ...
                            <Label className="text-sm/6 "> By selecting this, you agree to our{' '}
                                <Link to="/privacy-policy" className="font-semibold text-indigo-400"> privacy&nbsp;policy </Link> .
                            </Label>
                        </Field>
                    </div>
                    <div className="mt-10">
                        <button type="submit" className={`block w-full rounded-md px-3.5 py-2.5 text-center text-sm font-semibold text-white shadow-xs  ${!agreed ? 'bg-indigo-100' : 'bg-indigo-500 hover:bg-indigo-400'} focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600`} disabled={!agreed}> Submit </button>
                    </div>
                </form>
            </div>
